...
final class TextDeliveryService {
  ...
  /// Saved before the first clipboard write of a take. Kept across rapid
  ...
  func deliver(
    ...
    guard attempt == .pasted, let target else {
      // User needs the transcript on the clipboard to paste themselves.
    ...
    if pasted {
      // Cmd+V is processed asynchronously (Handy #502). Restoring immediately
      // makes the target paste the previous clipboard instead of the take.
      let restoreNs: UInt64 =
        isChromiumHost(target) || isTerminalHost(target)
        ? 1_800_000_000
        : 1_200_000_000
      scheduleClipboardRestore(
        after: restoreNs,
        ifChangeCount: pasteChangeCount
      )
      return .pasted
    }
  ...
  private func rememberClipboardIfNeeded() {
    if originalClipboard == nil {
      originalClipboard = PasteboardCapture.snapshot()
    }
    restoreTask?.cancel()
    restoreTask = nil
  }

  private func abandonClipboardRestore() {
    restoreTask?.cancel()
    restoreTask = nil
  ...
  private func scheduleClipboardRestore(
    ...
    restoreTask = Task { @MainActor [weak self] in
      ...
      if NSPasteboard.general.changeCount == expectedChangeCount,
        let original = self.originalClipboard
      {
        original.restore()
      }
      self.originalClipboard = nil
...
/// Snapshot of pasteboard items so a successful Cmd+V paste can put the user's
/// previous clipboard back (images, files, and password-manager types included).
struct PasteboardCapture: Equatable {
  ...
  static func snapshot(
    ...
    return PasteboardCapture(items: captured)
  }

  func restore(to pasteboard: NSPasteboard = .general) {
    pasteboard.clearContents()
    guard !items.isEmpty else { return }
    let objects: [NSPasteboardItem] = items.map { values in
